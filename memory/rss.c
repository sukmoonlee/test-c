...
void allocate_4k_pages_real(int count) {
    int ret;
    ret = system("cp -f /proc/meminfo meminfo.before");
	printf("Attempting to allocate %i Mebibytes of resident memory...\n", count);
    ...
    ret = system("ps aux |grep rss |grep -v grep|grep -v bash");
